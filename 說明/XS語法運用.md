| web-scraper-order | web-scraper-start-url | title | title-href | contect |
| --- | --- | --- | --- | --- |
| 1749713383-1 | https://www.xq.com.tw/lesson/xspractice/ | 排行語法 | https://www.xq.com.tw/lesson/xspractice/%E6%8E%92%E8%A1%8C%E8%AA%9E%E6%B3%95/ | 排行語法2025-06-09 18:42:12複製連結內容目錄語法基礎範例rank 語法的注意點「排行語法」讓使用者可直接在選股腳本中撰寫排行條件，無需額外撰寫函數腳本，並可靈活運用不同的排行名次作為篩選條件。

 
語法
基礎範例
複製1rank myRank begin
2Value1 = Average(Close, 10);
3retval = (Close - Value1);
4end;
5
6if myRank.pos <= 100 then ret = 1;
7OutputField1(myRank.value);
...
複製1var: myRank(0);
2rank myRank begin
3Value1 = Average(Close, 10);
4retval = (Close - Value1);
5end;
 

(O) 正確的案例：

複製1rank myRank1 begin
2retval = Close;
3end;
4rank myRank2 begin
5retval = Volume;
...
2retval = Close;
3end;
4if myRank.pos[1] <> myRank.pos then ret = 1;
5//篩選出前期排行與當期排行不同的商品
 
...
複製1//大到小
2rank myRank1 desc begin
3retval = Close;
4end;
5
6//小到大
7rank myRank2 asc begin
8retval = Close;
9end;
 
...
2Input: len(10);
3rank myRank begin
4Value1 = Average(Close, len);
5retval = (Close - Value1);
6end;
 

(O) 正確的案例： 
複製1//編譯時禁止 rank 內宣告相同名稱的變數
2var: _price(0);
3_price = Open;
4rank myRank begin
5var: _price(0);
6_price = Average(Close, 10);
7retval = (Close - _price);
8end;
9// rank 物件內 _price 會是 10 期收盤價移動平均值，物件外的 _price 會是開盤價
 
...
複製1//收盤價排行
2
3rank rank_close begin
4retval = close;
5end;
6
7//成交量排行
8
9rank rank_volume begin
10retval = volume;
11end;
12
13//上述兩個排行的相關性
14
15rank rank_alpha_13 begin
16retval = Covariance(rank_close.pr, rank_volume.pr, 5);
17end;
18ret = 1;
19
20OutputField1(rank_alpha_13.value, "相關性"); |
| 1749713385-2 | https://www.xq.com.tw/lesson/xspractice/ | 商品清單功能 | https://www.xq.com.tw/lesson/xspractice/%E5%95%86%E5%93%81%E6%B8%85%E5%96%AE%E5%8A%9F%E8%83%BD/ | 商品清單功能2025-06-09 18:41:52複製連結內容目錄語法說明透過 Input 宣告清單透過 Group 宣告清單指標腳本的清單選單搭配清單使用的函數使用範例盤中漲幅排行 [警示腳本]指數成分股的營收加總 [指標腳本]對應選擇權的成交金額加總 [指標腳本]目前 XS 語法可透過 GetSymbolField 取得不同商品的資料欄位，但在指定商品的狀況下僅能透過 Input 參數或直接輸入商品代號字串，當需要處理大量商品時，腳本撰寫會變得相對複雜 。
...
語法說明
透過 Input 宣告清單
複製1Input: myGroup(Group);
2Var: _TF(False);
3
4Value1 = GetSymbolField(myGroup[1], "Close", "D");
5_TF = GetSymbolInfo(myGroup[1], "可放空");
 
...
2Var: _TF(False);
3
4myGroup = GetSymbolGroup("TSE23.TW", "成分股");
5
6Value1 = GetSymbolField(myGroup[1], "Close", "D");
7_TF = GetSymbolInfo(myGroup[1], "可放空");
 
...
2value1 = GetSymbolField(cb_id, "Close");
3value2 = GetSymbolField(cb_id, "Volume");
4plot1(value1, "CB走勢");
5plot2(value2, "CB成交量");
 
...
 
搭配清單使用的函數
複製1Value1 = GroupSize(myGroup);
 

GroupSize 會回傳商品清單包含的數量 。 可以此數值避免取用到超出陣列範圍的資料而導致錯誤 。
使用範例
盤中漲幅排行 [警示腳本]
複製1input: _S(group, "排行股票");
2Array: rankRT[2000, 2](-9999);
3
4SetTotalBar(10);
5value1 = GroupSize(_S);
6
7//將清單的 商品代碼 以及 漲跌幅 的數值放入陣列中
8
9for value2 = 1 to value1 begin
10value3 = 100 * (getsymbolField(_S[value2], "Close", default = 0)
11- getsymbolField(_S[value2], "參考價", "D", default = 0))
12/ getsymbolField(_S[value2], "參考價", "D", default = 1);
13rankRT[value2, 1] = strtonum(leftStr(_S[value2], 4));
14rankRT[value2, 2] = value3;
15end;
16
17//將陣列依據漲跌幅排序後印出
18Array_Sort2d(rankRT, 1, value1, 2, false);
19print("==========漲跌幅前10==========");
20print("日期時間：", numtostr(datetime, 0));
21for value2 = 1 to 10 begin
22print(text("排名", numtostr(value2, 0), "商品: "),
23text(numtostr(rankRT[value2, 1], 0), ".TW"),
24" / 漲跌幅: ", rankRT[value2, 2]);
25end;
26
27print("==============================");
28if symbol = text(numtostr(rankRT[1, 1], 0), ".TW") then ret = 1;
 

上述腳本中宣告了一個清單 （ 設定為需要盤中即時排序的商品 ） 和陣列，並將清單內的商品代號和漲跌幅存入陣列後進行排序 。
指數成分股的營收加總 [指標腳本]
複製1group: _symbolGroup();
2var: _sum(0), _num(0);
3
4_symbolGroup = GetSymbolGroup("成分股");
5value1 = GroupSize(_symbolGroup);
6
7_sum = 0;
8_num = 0;
9for value2 = 1 to value1 begin
10if CheckSymbolField(_symbolGroup[value2], "月營收", "M") then begin
11_sum += GetSymbolField(_symbolGroup[value2], "月營收", "M");
12_num += 1;
13end;
14end;
15
16plot1(_sum);
17SetPlotLabel(1, "成分股月營收");
18plot2(_num);
19SetPlotLabel(2, "有月營收家數");
20plot3(value1);
21SetPlotLabel(3, "成分股家數");
 
...
對應選擇權的成交金額加總 [指標腳本]
複製1group: _list();
2var: _uSum(0), _dSum(0), _count(0), _cp("");
3
4_list = getsymbolgroup("選擇權");
5
6_uSum = 0; //買權加總
7_dSum = 0; //賣權加總
8_count = 0;
9for value1 = 1 to groupSize(_list) begin
10if getsymbolField(_list[value1], "Date", "Tick", default:= 0) = getfield("Date", default:= -999999) then begin
11_cp = getsymbolinfo(_list[value1], "買賣權");
12if _cp = "CALL" then _uSum += getsymbolField(_list[value1], "Close", "Tick", default:=0) * getsymbolField(_list[value1], "Volume", "Tick", default:=0) * 2000;
13if _cp = "PUT" then _dSum += getsymbolField(_list[value1], "Close", "Tick", default:=0) * getsymbolField(_list[value1], "Volume", "Tick", default:=0) * 2000;
14end;
15end;
16
17plot1(_uSum, "近一筆買權權利金加總金額");
18plot2(_dSum, "近一筆賣權權利金加總金額");
 
...
| 1749713387-3 | https://www.xq.com.tw/lesson/xspractice/ | GetField 預設值 | https://www.xq.com.tw/lesson/xspractice/getfield-%E9%A0%90%E8%A8%AD%E5%80%BC/ | GetField 預設值2025-06-09 18:41:13複製連結內容目錄語法說明defaultCheckFieldIsSupportField使用者在取得資料欄位時，有時會遇到資料尚未更新或欄位缺失的情況，這可能導致回測或執行時發生錯誤。 為避免此類問題，GetField 欄位新增了預設值參數，當資料缺失時，可回傳指定的預設值，確保策略順利執行。 除了預設值以外，還有其他函數可用來判斷欄位是否可用以及欄位名稱等參數是否正確。

 
語法說明
default
複製1Value1 = GetField("大戶持股比例", "W", default := 0); 
2Value2 = GetSymbolField("2330.TW", "大戶持股比例", "W", default := Value2[1]);
...
 
IsSupportField
複製1IsSupportField("月營收", "M");
2IsSupportSymbolField("TSE.TW", "月營收");
 
...
| 1749713389-4 | https://www.xq.com.tw/lesson/xspractice/ | XS美股新增欄位列表 | https://www.xq.com.tw/lesson/xspractice/xs%E7%BE%8E%E8%82%A1%E6%96%B0%E5%A2%9E%E6%AC%84%E4%BD%8D%E5%88%97%E8%A1%A8/ | XS美股新增欄位列表2023-10-26 16:34:44複製連結
...
| 1749713392-5 | https://www.xq.com.tw/lesson/xspractice/ | XS美股欄位的使用介紹 | https://www.xq.com.tw/lesson/xspractice/xs%E7%BE%8E%E8%82%A1%E6%AC%84%E4%BD%8D%E7%9A%84%E4%BD%BF%E7%94%A8%E4%BB%8B%E7%B4%B9/ | XS美股欄位的使用介紹2023-10-26 16:36:31複製連結
...
複製1Retval = GetField("殖利率", "D");
第二步：在 XS 編輯器，新增「殖利率相關資訊」選股腳本，程式碼如下
複製1Ret = 1;
2outputField1(GetField("殖利率", "D"),"殖利率");
3outputField2(GetFielddate("殖利率", "D"),"殖利率資料日期");
4outputField3(GetField("現金股利", "Y"),"現金股利");
5outputField4(GetFielddate("現金股利", "Y"),"現金股利資料日期");
6outputField5(GetField("總市值(元)", "D"),"總市值(元)");
7outputField6(GetFielddate("總市值(元)", "D"),"總市值(元)資料日期");
...
複製1//因為繪圖設定→查價視窗→數值顯示→%，會自動 × 100，所以該數值要 / 100，才會在顯示正確 % 數值。
2plot1(GetField("機構持股比重", "Q")/100,"比重",Checkbox:=1);
3plot2(GetField("機構持股", "Q"),"數值",Checkbox:=0);
...
延伸閱讀：
 XS美股欄位表 進一步了解美股分析模組 |
| 1749713394-6 | https://www.xq.com.tw/lesson/xspractice/ | 取得資料欄位時的「對位問題」 | https://www.xq.com.tw/lesson/xspractice/%E5%8F%96%E5%BE%97%E8%B3%87%E6%96%99%E6%AC%84%E4%BD%8D%E6%99%82%E7%9A%84%E3%80%8C%E5%B0%8D%E4%BD%8D%E5%95%8F%E9%A1%8C%E3%80%8D/ | 取得資料欄位時的「對位問題」2023-10-12 09:23:40複製連結
...
複製1// 計算 近 4 期的月營收平均
2Value1 = GetField("月營收", "M");
3Value2 = Average(Value1+Value1[1]+Value1[2]+Value1[3], 4);
...
我們下次見！ |
| 1749713396-7 | https://www.xq.com.tw/lesson/xspractice/ | 選股中心創掛牌新高與大單欄位的應用 | https://www.xq.com.tw/lesson/xspractice/%E9%81%B8%E8%82%A1%E4%B8%AD%E5%BF%83%E5%89%B5%E6%8E%9B%E7%89%8C%E6%96%B0%E9%AB%98%E8%88%87%E5%A4%A7%E5%96%AE%E6%AC%84%E4%BD%8D%E7%9A%84%E6%87%89%E7%94%A8/ | 選股中心創掛牌新高與大單欄位的應用2021-06-25 09:28:36複製連結大家好，在7.05.05與3.05.05版本，選股中心新增以下兩個項目，在本文中將為大家做介紹：
...
複製1value801 = GetField("買進特大單量", "D") + GetField("買進大單量", "D");
2value802 = GetField("賣出特大單量", "D") + GetField("賣出大單量", "D");
3value888 = value801 - value802;
4//大單買超 = (買進特大單+買進大單)-(賣出特大單+賣出大單) > 0
5if value888 > 0 then ret = 1;
6outputField1(value888,"大單買超由大到小排名",order:=1);


以下這張表格，是此次新增的選股欄位與說明表格，詳細的介紹可點選連結查閱，有了這些選股欄位，投資人就能在盤前先做功課，運用選股中心篩選出來的結果，觀察近期大戶資金布局在何處，搶得先機。
選股欄位名稱說明內盤成交次數/外盤成交次數當日的成交筆數/以內盤成交的成交筆數/以外盤成交的成交筆數買進特大單量/買進大單量/買進中單量/買進小單量依照單筆交易金額分級，統計當日以外盤成交的特大單/大單/中單/小單的成交量(張數)賣出特大單量/賣出大單量/賣出中單量/賣出小單量依照單筆交易金額分級，統計當日以內盤成交的特大單/大單/中單/小單的成交量(張數)買進特大單金額/買進大單金額/買進中單金額/買進小單金額依照單筆交易金額分級，統計當日以外盤成交的特大單/大單/中單/小單的成交金額(元)賣出特大單金額/賣出大單金額/賣出中單金額/賣出小單金額依照單筆交易金額分級，統計當日以內盤成交的特大單/大單/中單/小單的成交金額(元)買進特大單成交次數/買進大單成交次數/買進中單成交次數/買進小單成交次數依照單筆交易金額分級，統計當日以外盤成交的特大單/大單/中單/小單的成交筆數賣出特大單成交次數/賣出大單成交次數/賣出中單成交次數/賣出小單成交次數依照單筆交易金額分級，統計當日以內盤成交的特大單/大單/中單/小單的成交筆數以上就是在 7.05.05 / 3.05.05 版本新增的XS選股相關欄位與語法應用，感謝大家的聆聽，XS團隊會持續規劃增強相關語法功能與應用，陪伴投資人一同茁壯成長，希望能助投資人一臂之力，以利使用者在投資道路上更加順心。 |
| 1749713398-8 | https://www.xq.com.tw/lesson/xspractice/ | 台股逐筆撮合的連續成交Tick序列 | https://www.xq.com.tw/lesson/xspractice/%E5%8F%B0%E8%82%A1%E9%80%90%E7%AD%86%E6%92%AE%E5%90%88%E7%9A%84%E7%A9%BF%E5%83%B9tick/ | 台股逐筆撮合的連續成交Tick序列2020-07-08 17:50:08複製連結大家好。在上一篇文章「Tick欄位的應用」，我們說明了如何從腳本內取得「Tick」頻率的欄位。在這一篇文章內我們要介紹台股逐筆撮合制度上線後所產生的另外一種成交方式，我們稱之為「連續成交序列」。
...
複製1value1 = GetField("TickGroup", "Tick");
...
複製1input: filterMode(1, "篩選方式", inputkind:=dict(["買盤",1], ["賣盤",-1]));
...
ReadTicks讀回的Tick資料會存放在tick_array內，最新的一筆是第一個橫列(row), 前一筆是第二個row，以下類推。每一個row內包含11個欄位(column)，內容分別是：
 第1個column是日期 第2個column是時間 第3個column是成交價 第4個column是成交量 第5個column是內外盤註記 第6個column是這一筆Tick的編號(SeqNo) 如果ReadTicks遇到多筆連續成交序列時，他會把這些Ticks都合併成1個row，第7個column存放的是這個連續成交序列總共有多少筆 如果這是連續成交序列的話，第8個column存放的是這個連續成交序列的第一筆的位置，第9個column存放的則是這個連續成交序列最後一筆的位置 第10個column存放的是這些連續成交序列的總成交量，如果這不是連續成交序列的話，這個column存放的則是這一筆的成交量(跟第四個column相同 第11個column存放的是這些連續成交序列的總成交金額(元)，如果這不是連續成交序列的話，這個column存放的則是這一筆的成交金額所以透過ReadTicks之後，我們就可以很簡單的檢查這一筆資料是否符合我們要篩選的規則
複製1if tick_array[idx, 5] = filterMode and tick_array[idx, 10] >= filterVolume then ...
...
| 1749713400-9 | https://www.xq.com.tw/lesson/xspractice/ | Tick欄位的應用 | https://www.xq.com.tw/lesson/xspractice/tick%E6%AC%84%E4%BD%8D%E7%9A%84%E6%87%89%E7%94%A8/ | Tick欄位的應用2020-07-08 17:45:36複製連結在上一篇文章「盤中即時資料欄位的應用」，我們介紹了如何在腳本內引用「即時資料欄位」來繪製指標或是執行警示，在這一篇文章內我們將為大家介紹另外一種盤中會即時更新的資料: Tick資料。
...
複製1value1 = GetField("Date", "Tick");   // 成交日期，例如20200611 (2020年6月11日)
2value2 = GetField("Time", "Tick");   // 成交時間，例如103011 (10點30分11秒)
3value3 = GetField("Close", "Tick");  // 成交價格
4value4 = GetField("Volume", "Tick"); // 成交單量
5value5 = GetField("BidAskFlag", "Tick"); // 內外盤標記: 1代表外盤成交(紅色), -1代表內盤成交(綠色), 0代表中立
6value6 = GetField("BidPrice", "Tick"); // 買進價格
7value7 = GetField("AskPrice", "Tick"); // 賣出價格
8value8 = GetField("SeqNo", "Tick");  // 資料編號, 每個交易日從1開始編制, 第一筆是1, 第二筆是2, 以下類推
9
10value22 = GetField("Close", "Tick")[1]; // 前一筆成交價格
11value23 = GetField("Close", "Tick")[2]; // 前兩筆成交價格
透過Tick資料，我們從腳本內就可以更掌握目前的行情資料。我們來看一個大單篩選的警示範例，假如我們希望商品出現單筆成交量大於100張的成交時就通知我們的話，可以用底下這個警示腳本來完成：
複製1input: filterMode(1, "篩選方式", inputkind:=dict(["買盤",1], ["賣盤",-1]));
2input: filterVolume(100, "大單門檻");
3
4value1 = GetField("Time", "Tick"); // 時間
5value2 = GetField("Close", "Tick"); // 價格
6value3 = GetField("Volume", "Tick"); // 單量
7value4 = GetField("BidAskFlag", "Tick"); // 外盤=1, 內盤=-1
8
9if value4 = filterMode and value3 >= filterVolume then ret=1;
...
從上面這張圖內我們可以觀察到XS洗價的執行方式：
 啟動逐筆洗價時，當系統收到成交資料時就會觸發K棒的洗價， 一般而言每收到一筆成交就會觸發一次K棒的洗價，請注意上圖內的洗價時間是示意資料，實際洗價時間會因為電腦CPU，指定的洗價速度等因素而有差異， 可是如果快市的話，就有可能不是每一筆Tick都觸發一次洗價，而是好幾筆Tick才觸發一次洗價，例如在上面這張圖內, Tick編號#3跟Tick編號#4兩筆資料的間隔時間很短，所以觸發洗價時已經收到兩筆Tick了(編號#3跟編號#4)， 報價欄位所回傳的是目前最新的行情，在上面這個情形時就是編號#4的數值，所以大部分的時候報價欄位的數值會等於最新一筆Tick的資料， 可是有可能在電腦洗價時又收到了更多的成交資料，此時報價欄位會更新，q_Last就不一定會跟洗價時K棒的Close是一樣的數值，例如在上面這張圖內，執行第四次洗價時如收到了編號#6的Tick的話, q_Last會被更新成編號#6的資料，此時就會跟第四次洗價時所抓到的價格不一樣， 如果在洗價時去抓取Tick資料的話，則系統會保證此時抓到的Tick資料跟洗價當時的K棒內容是一致的。例如第四次洗價時抓到的Tick會是編號#5的Tick，縱使洗價當時編號#6的Tick已經收到了， 腳本可以在洗價當時透過讀取Tick資料的方式來抓到兩次洗價之間的所有Ticks：例如在第四次洗價時抓到的Tick是編號#5的Tick，此時腳本可以再抓取「前一筆」Tick，就會抓到編號#4的Tick，K棒洗價的速度可以透過系統的設定功能來指定，如果調整成最快的話約是125ms會執行一次洗價，如果調整成最慢的話則是每1秒執行一次洗價，使用者可以依照腳本複雜度，CPU的等級等來決定合適的數值。
...
2input: filterVolume(100, "大單門檻");
3
4var: intrabarpersist last_seqno(0);// 上次洗價時最後一筆Tick的SeqNo
5var: curr_seqno(0);// 這次洗價時最後一筆Tick的SeqNo
6
7if Date <> CurrentDate then return;// 只跑今日的資料
8
9curr_seqno = GetField("SeqNo", "Tick");// 最新一筆Tick編號
10
11if last_seqno = 0 then last_seqno = curr_seqno - 1; // 第一次洗價時只洗當時那一筆
12
13var: seq_no(0), offset(0);
14seq_no = curr_seqno;
15offset = 0;
16while seq_no > last_seqno begin
17
18  // 讀取Tick資料
19  value1 = GetField("Time", "Tick")[offset];
20  value2 = GetField("Close", "Tick")[offset];
21  value3 = GetField("Volume", "Tick")[offset];
22  value4 = GetField("BidAskFlag", "Tick")[offset];
23
24  if value4 = filterMode and value3 >= filterVolume then begin
25    ret=1;
26  end;
27
28  seq_no = seq_no - 1;
...
下次再見囉。 |
| 1749713402-10 | https://www.xq.com.tw/lesson/xspractice/ | 盤中即時資料欄位的應用 | https://www.xq.com.tw/lesson/xspractice/%E7%9B%A4%E4%B8%AD%E5%8D%B3%E6%99%82%E8%B3%87%E6%96%99%E6%AC%84%E4%BD%8D%E7%9A%84%E6%87%89%E7%94%A8/ | 盤中即時資料欄位的應用2020-07-08 17:45:07複製連結在這一篇文章內我們會跟大家介紹另外一個7.02/3.02版所新增的XS功能：「盤中即時資料欄位」。
...
複製1value1 = GetField("內盤量", "1"); // 取得1分鐘頻率的數值
2value2 = GetField("內盤量", "2"); // 取得2分鐘頻率的數值
3value3 = GetField("內盤量", "5"); // 取得5分鐘頻率的數值
4value4 = GetField("內盤量");      // 不指定頻率: 依照目前執行的頻率
...
欄位名稱說明成交金額當分鐘的成交金額均價開盤到目前為止的成交均價內盤量/外盤量當分鐘的內盤成交量/外盤成交量上漲量/下跌量當分鐘的上漲成交量/下跌成交量估計量到目前為止的當日估計量量比估計量 / 昨日整日的成交量總成交次數/內盤成交次數/外盤成交次數當分鐘的成交筆數/以內盤成交的成交筆數/以外盤成交的成交筆數買進特大單量/買進大單量/買進中單量/買進小單量依照單筆交易金額分級，統計當分鐘以外盤成交的特大單/大單/中單/小單的成交量(張數)賣出特大單量/賣出大單量/賣出中單量/賣出小單量依照單筆交易金額分級，統計當分鐘以內盤成交的特大單/大單/中單/小單的成交量(張數)買進特大單金額/買進大單金額/買進中單金額/買進小單金額依照單筆交易金額分級，統計當分鐘以外盤成交的特大單/大單/中單/小單的成交金額(元)賣出特大單金額/賣出大單金額/賣出中單金額/賣出小單金額依照單筆交易金額分級，統計當分鐘以內盤成交的特大單/大單/中單/小單的成交金額(元)買進特大單成交次數/買進大單成交次數/買進中單成交次數/買進小單成交次數依照單筆交易金額分級，統計當分鐘以外盤成交的特大單/大單/中單/小單的成交筆數賣出特大單成交次數/賣出大單成交次數/賣出中單成交次數/賣出小單成交次數依照單筆交易金額分級，統計當分鐘以內盤成交的特大單/大單/中單/小單的成交筆數累計委買/累計委賣/累委買筆/累委賣筆由交易所公布，開盤到目前的累計委買委賣張數(口數)，以及累計委買委賣筆數累買成筆/累賣成筆由交易所公布，開盤到目前的以委買成交的筆數以及以委賣成交的筆數，支援期貨／選擇權累計成交/累成交筆由交易所公布，開盤到目前的累計成交數量以及成交筆數漲停家數/跌停家數/上漲家數/下跌家數目前最新成交價是漲停/跌停/上漲/下跌的家數統計內盤家數/外盤家數當分鐘的內盤家數/外盤家數基差當分鐘現貨價格與期貨價格之間的差額Delta/Gamma/Theta/Vega/RHO權證/選擇權的希臘數字理論價目前最新的理論價格波動率當分鐘的波動率隱含波動率當分鐘的隱含波動率買權成交量/賣權成交量開盤到現在的買權/賣權成交量總和。夜盤商品不納入計算。 

看完欄位清單之後，那我們就使用這些欄位來做幾個簡單的應用。

首先開啟IDE，新增一個指標腳本，取名為「當日賺賠」，輸入以下的程式碼：
複製1value1 = GetField("均價");
2value2 = weightedclose - value1;
3
4plot1(value2, "賺賠");
...
複製1value1 = GetField("買進特大單量", "1") - GetField("賣出特大單量", "1"); 
2if value1 > 0 then ret=1;
...
| 1749713404-11 | https://www.xq.com.tw/lesson/xspractice/ | 利用InputKind函數製作跨頻率週期的選取介面 | https://www.xq.com.tw/lesson/xspractice/%E5%88%A9%E7%94%A8inputkind%E5%87%BD%E6%95%B8%E8%A3%BD%E4%BD%9C%E8%B7%A8%E9%A0%BB%E7%8E%87%E9%80%B1%E6%9C%9F%E7%9A%84%E9%81%B8%E5%8F%96%E4%BB%8B%E9%9D%A2/ | 利用InputKind函數製作跨頻率週期的選取介面2018-12-19 20:43:30複製連結
...
複製1 // 跨頻率KD指標，預設跨頻率為30分
2 // 不支援大頻率跨小頻率，例如：
3 // 不支援主頻率60分鐘，跨頻率計算30分鐘KD技術指標。
4 //
5 input:    Length(9, "天數"), RSVt(3, "RSVt權數"), Kt(3, "Kt權數"),
6         FreqType("30", "跨頻率週期", inputkind:=dict(["1分鐘","1"],["5分鐘","5"],["10分鐘","10"],["15分鐘","15"],["30分鐘","30"],["60分鐘","60"],["日","D"],["還原日","AD"]));
7 variable: rsv(0), k(0), _d(0);
8 
9 if barfreq <> "Tick" and barfreq <> "Min" and barfreq <> "D" and barfreq <> "AD" then raiseruntimeerror("此範例僅支援分鐘、日與還原日頻率");
10 
11 xfMin_stochastic(FreqType, Length, RSVt, Kt, rsv, k, _d);
12 
13 Plot1(k, "分鐘與日K(%)");
14 Plot2(_d, "分鐘與日D(%)");
15 
16 // 防呆，大頻率跨小頻率時，在線圖秀不支援
17 //
18 switch (FreqType)
19 begin
20     case  "1":
21         if barfreq <> "Tick" and barfreq <> "Min" then raiseruntimeerror("不支援大頻率跨小頻率：主頻率大於1分鐘");
22         if barinterval <> 1 then raiseruntimeerror("不支援大頻率跨小頻率：主頻率大於1分鐘");
23         setplotlabel(1, "1分K(%)");
24         setplotlabel(2, "1分D(%)");
25 
26     case  "5":
27         if barfreq <> "Tick" and barfreq <> "Min" then raiseruntimeerror("不支援大頻率跨小頻率：主頻率大於5分鐘");
28         if barinterval <> 1 and barinterval <> 2 and barinterval <> 3 and barinterval <> 5 then raiseruntimeerror("不支援大頻率跨小頻率：主頻率大於5分鐘");
29         setplotlabel(1, "5分K(%)");
30         setplotlabel(2, "5分D(%)");
31 
32     case "10":
33         if barfreq <> "Tick" and barfreq <> "Min" then raiseruntimeerror("不支援大頻率跨小頻率：主頻率大於10分鐘");
34         if barinterval <> 1 and barinterval <> 2 and barinterval <> 3 and barinterval <> 5 and barinterval <> 10 then raiseruntimeerror("不支援大頻率跨小頻率：主頻率大於10分鐘");
35         setplotlabel(1, "10分K(%)");
36         setplotlabel(2, "10分D(%)");
37 
38     case "15":
39         if barfreq <> "Tick" and barfreq <> "Min" then raiseruntimeerror("不支援大頻率跨小頻率：主頻率大於15分鐘");
40         if barinterval <> 1 and barinterval <> 2 and barinterval <> 3 and barinterval <> 5 and barinterval <> 10 and barinterval <> 15 then raiseruntimeerror("不支援大頻率跨小頻率：主頻率大於15分鐘");
41         setplotlabel(1, "15分K(%)");
42         setplotlabel(2, "15分D(%)");
43         
44     case "30":
45         if barfreq <> "Tick" and barfreq <> "Min" then raiseruntimeerror("不支援大頻率跨小頻率：主頻率大於30分鐘");
46         if barinterval <> 1 and barinterval <> 2 and barinterval <> 3 and barinterval <> 5 and barinterval <> 10 and barinterval <> 15 and barinterval <> 20 and barinterval <> 30 then raiseruntimeerror("不支援大頻率跨小頻率：主頻率大於30分鐘");
47         setplotlabel(1, "30分K(%)");
48         setplotlabel(2, "30分D(%)");
49 
50     case "60":
51         if barfreq <> "Tick" and barfreq <> "Min" then raiseruntimeerror("不支援大頻率跨小頻率：主頻率大於60分鐘");
52         if barinterval <> 1 and barinterval <> 2 and barinterval <> 3 and barinterval <> 5 and barinterval <> 10 and barinterval <> 15 and barinterval <> 20 and barinterval <> 30 and barinterval <> 45 and barinterval <> 60 then raiseruntimeerror("不支援大頻率跨小頻率：主頻率大於60分鐘");
53         setplotlabel(1, "60分K(%)");
54         setplotlabel(2, "60分D(%)");
55     
56     case "D":
57         if barfreq <> "Tick" and barfreq <> "Min" and barfreq <> "D" and barfreq <> "AD" then raiseruntimeerror("不支援大頻率跨小頻率：主頻率大於日");
58         setplotlabel(1, "日K(%)");
59         setplotlabel(2, "日D(%)");
60 
61     case "AD":
62         if barfreq <> "Tick" and barfreq <> "Min" and barfreq <> "D" and barfreq <> "AD" then raiseruntimeerror("不支援大頻率跨小頻率：主頻率大於還原日");
63         setplotlabel(1, "還原日K(%)");
64         setplotlabel(2, "還原日D(%)");        
65 
66 end; |
| 1749713407-12 | https://www.xq.com.tw/lesson/xspractice/ | 利用GetSymbolInfo函數計算選擇權的希臘字母Delta | https://www.xq.com.tw/lesson/xspractice/%E5%88%A9%E7%94%A8getsymbolinfo%E5%87%BD%E6%95%B8%E8%A8%88%E7%AE%97%E9%81%B8%E6%93%87%E6%AC%8A%E7%9A%84%E5%B8%8C%E8%87%98%E5%AD%97%E6%AF%8Ddelta/ | 利用GetSymbolInfo函數計算選擇權的希臘字母Delta2018-12-19 20:42:24複製連結市場上投資工具五花八門，常見的就有股票、期貨、選擇權、權證等等。時至今日，選擇權和權證已經是很普遍使用的投資標的。評估一檔股票值不值得投資，我們會用基本面或技術面等條件篩選，利用XS都可以輕鬆做到。在投資選擇權時，用來評價選擇權風險就是所謂的希臘字母（Delta、Gamma、Theta、Vega、Rho）。這項功課，現在XS也能幫你完成。
...
複製1買賣權=midstr(symbol,6,1); //利用商品名稱取得
...
複製1getsymbolfield("Underlying","收盤價");
...
| 1749713409-13 | https://www.xq.com.tw/lesson/xspractice/ | 關於背離的寫法 | https://www.xq.com.tw/lesson/xspractice/%E9%97%9C%E6%96%BC%E8%83%8C%E9%9B%A2%E7%9A%84%E5%AF%AB%E6%B3%95/ | 關於背離的寫法2016-03-10 17:08:03複製連結有讀者問到，XS有沒有辦法判斷指標是否背離。今天這一篇文章內就來跟大家探討背離的寫法。
...
小編運用這個原理，寫出以下的背離函數：
複製1input:price(numericsimple),index1(numericsimple),length(numericsimple);
2if length<5
3then raiseruntimeerror("計算期別請超過五期");
4
5value1=linearregslope(price,length);
6value2=linearregslope(index1,length);
7
8if value1>0 and value2<0
9then deviate=-1
10else
11　　if value1<0 and value2>0
12　　then deviate=1
13　　else
14　　　　deviate=0;
...
複製1value1=rsi(close,10);
2if deviate(close,value1,10)=1
3then ret=1;
有了這個函數，我們就可以隨時把兩個時間序列拿出來檢定是否有背離的現象了。

今天就為大家介紹到此，有任何建議的話歡迎隨時跟我們講喔。下次見。 |
| 1749713411-14 | https://www.xq.com.tw/lesson/xspractice/ | 私房交易策略之：強勢股整理結束 | https://www.xq.com.tw/lesson/xspractice/%E7%A7%81%E6%88%BF%E4%BA%A4%E6%98%93%E7%AD%96%E7%95%A5%E4%B9%8B%E5%BC%B7%E5%8B%A2%E8%82%A1%E6%95%B4%E7%90%86%E7%B5%90%E6%9D%9F/ | 私房交易策略之：強勢股整理結束2016-03-10 17:03:55複製連結作為一個操作者，我們對那些來不及上車的強勢股，也往往有莫可奈何的心情，
...
複製1Value1 = GetBarOffset(20151014);
如果今天是2015年10月15日，那上面的程式執行時Value1就會是1，代表是前一根bar。

有了這個函數，我就來寫出上面所提到的這個腳本
複製1input: stardate(20150824);
2input:ratio(30);
3input:ratio1(7);
4input:ratio2(2);
5setinputname(1,"輸入上漲起始日");
6setinputname(2,"輸入上漲最低幅度");
7setinputname(3,"輸入最小拉回幅度");
8setinputname(4,"今日最低漲幅");
9setfirstbardate(stardate);
10value1=getbaroffset(stardate);//找出輸入的日期是在第幾根bar
11value2=highest(high[1],value1+1);//找出這一波的最高點
12
13condition1=false;
14condition2=false;
15
16if value2>=close[value1]*(1+ratio/100)//計算波段漲幅有沒有符合要求
17then condition1=true;
18
19if value2>=close[1]*(1+ratio1/100)//計算拉回的幅度夠不夠要求
20then condition2=true;
21
22if nthhighestbar(1,high,10)>=5//從最高點到今天超過五根bar
23then begin
24if condition1 and condition2 and close>=close[1]*(1+ratio2/100)
25then ret=1;
26end;
...
| 1749713413-15 | https://www.xq.com.tw/lesson/xspractice/ | 用XS寫籌碼集中度的選股策略 | https://www.xq.com.tw/lesson/xspractice/%E7%94%A8xs%E5%AF%AB%E7%B1%8C%E7%A2%BC%E9%9B%86%E4%B8%AD%E5%BA%A6%E7%9A%84%E9%81%B8%E8%82%A1%E7%AD%96%E7%95%A5/ | 用XS寫籌碼集中度的選股策略2016-03-10 16:59:35複製連結有朋友問到，在XS內有沒有籌碼相關的欄位，如果有的話如何運用。在這一篇文章內，我們就來跟大家介紹一下主力買賣超張數這個欄位，以及如何運用這個欄位來估算籌碼集中度。

首先，我們設計一個計算籌碼集中度的自訂函數，把他叫做BS，取Buysell(買賣超)的簡寫。不知道如何設定自訂函數的使用者，可以先複習一下計算區間漲跌幅的自訂函數這一篇文章：
複製1input:days(numericsimple,"計算天數");
2value1=GetField("主力買賣超張數");
3value2=summation(volume,days);
4value3=summation(value1,days);
5if value2<>0
6then value4=value3/value2*100;
7bs=value4;
接下來我用這個函數寫了下面這個選股腳本:
複製1input:days(10,"天數");
2input:ratio(5,"最低百分比");
3input:percent(10,"漲幅上限");
4value1=bs(days);
5if close[days-1]<>0 and close>close[days-1]
6then value2=(close-close[days-1])/close[days-1]*100;
7if value1>ratio and value2<percent 
8then ret=1;
9
10outputfield(1,value1,0,"籌碼集中度");
11outputfield(2,value2,1,"區間漲幅%");
...
| 1749713415-16 | https://www.xq.com.tw/lesson/xspractice/ | 那些股票中長紅之後還會續漲? | https://www.xq.com.tw/lesson/xspractice/%E9%82%A3%E4%BA%9B%E8%82%A1%E7%A5%A8%E4%B8%AD%E9%95%B7%E7%B4%85%E4%B9%8B%E5%BE%8C%E9%82%84%E6%9C%83%E7%BA%8C%E6%BC%B2/ | 那些股票中長紅之後還會續漲?2016-03-09 16:59:49複製連結江湖走久了，高手看的自然較多，有一門派代有才人出，這門派的心法只有一個~追強勢股。
...
我們可以寫一個選股腳本如下:
複製1if close>=close[1]*1.03 and volume>500
2then ret=1;
...
| 1749713417-17 | https://www.xq.com.tw/lesson/xspractice/ | 計算區間漲跌幅的自訂函數 | https://www.xq.com.tw/lesson/xspractice/%E8%A8%88%E7%AE%97%E5%8D%80%E9%96%93%E6%BC%B2%E8%B7%8C%E5%B9%85%E7%9A%84%E8%87%AA%E8%A8%82%E5%87%BD%E6%95%B8/ | 計算區間漲跌幅的自訂函數2016-03-10 17:11:39複製連結在今天這一篇文章內要來跟大家說明一下函數的用途，什麼是函數，怎麼寫一個函數，以及如何在其他的腳本內運用自訂函數。
...
複製1 
2 input:price(numeric);
3 input:startday(numeric);
4 input:endday(numeric);
5
6 value1=getbaroffset(startday);
7 value2=getbaroffset(endday);
8
9 value3=(price[value2]/price[value1])-1 ;
10 rangechange=value3*100;
...
複製1input:startday(20150702,"區間起始日");
2input:endday(20151002,"區間結束日");
3input:ratio(10,"最低漲幅");
4
5value1=rangechange(close,startday,endday);
6if value1>=ratio
7then ret=1;
8 
9value2=GetField("最新股本");
10value3=GetField("月營收年增率","M");
11value4=GetField("股價淨值比","D");
12outputfield(1,value1,1,"區間漲跌幅");
13outputfield(2,value2,0,"股本(億)");
14outputfield(3,value3,1,"月營收年增率");
15outputfield(4,value4,1,"股價淨值比");
16
...
| 1749713419-18 | https://www.xq.com.tw/lesson/xspractice/ | 選股欄位放大鏡：談OutputField跟GetFieldDate這兩個函數 | https://www.xq.com.tw/lesson/xspractice/%E9%81%B8%E8%82%A1%E6%AC%84%E4%BD%8D%E6%94%BE%E5%A4%A7%E9%8F%A1%EF%BC%9A%E8%AB%87outputfield%E8%B7%9Fgetfielddate%E9%80%99%E5%85%A9%E5%80%8B%E5%87%BD%E6%95%B8/ | 選股欄位放大鏡：談OutputField跟GetFieldDate這兩個函數2016-03-17 19:48:06複製連結在前一篇文章內我們談到了如何運用Print語法列印腳本的執行狀態，讓臭蟲無所遁形。由於目前選股腳本內還沒有辦法支援Print的語法，所以今天就來跟大家談一下小編在面對選股腳本的臭蟲時常用的幾個技倆。
...
複製1Value1 = GetField("月營收","M");
2
3ret = 1;
4outputfield(1, GetFieldDate("月營收","M"), "最新月份");
5outputfield(2, Value1);
6outputfield(3, Value1[1]);
7outputfield(4, Value1[2]);
...
複製1Var: mm(0);
2
3Value1 = getfield("月營收","M"); 
4mm = Month(GetFieldDate("月營收","M"));
5
6if mm=1 or mm=4 or mm=7 or mm=10
7then value2=Value1 * 3;
8 
9if mm=2 or mm=5 or mm=8 or mm=11
10then value2=Value1 * 2 + Value1[1];
11 
12if mm=3 or mm=6 or mm=9 or mm=12
13then value2=Value1+Value1[1]+Value1[2];
14 
15// 預估獲利(單位=百萬) = 季營收 * 毛利率 - 營業費用
16//
17value3 = value2 * GetField("營業毛利率","Q") - GetField("營業費用","Q");
18
19OutputField(1, mm, "營收月份");
20OutputField(2, Value1, "本月");
21OutputField(3, Value1[1], "本月[1]");
22OutputField(4, Value1[2], "本月[2]");
23OutputField(5, value2, "預估單季營收(億)");
24OutputField(6, value3 / 100, "預估單季本業獲利(億)");
25 
26ret = 1;
...
| 1749713421-19 | https://www.xq.com.tw/lesson/xspractice/ | 如何運用Print指令來抓程式的臭蟲 | https://www.xq.com.tw/lesson/xspractice/%E5%A6%82%E4%BD%95%E9%81%8B%E7%94%A8print%E6%8C%87%E4%BB%A4%E4%BE%86%E6%8A%93%E7%A8%8B%E5%BC%8F%E7%9A%84%E8%87%AD%E8%9F%B2/ | 如何運用Print指令來抓程式的臭蟲2016-03-17 18:14:21複製連結大家在寫腳本的時候，有沒有常常跟小編一樣，遇到跑出來的結果跟原先預期不一樣的情形？今天小編就要來跟大家介紹一個抓臭蟲(Bug)的工具：這個工具就叫做Print指令。Print指令除了可以用來抓臭蟲之外，他也可以幫我們把腳本中運算完的數據輸出成文字檔，方便我們把運算結果儲存下來或是再做其他的應用。

首先來介紹基本的語法：
複製1Value1 = Average(Close, 5);
2Print("Date=", Date, "Close=", Close, "Value1", Value1);
...
複製1print(file("C:\print\"),date,symbol,close);
...
複製1print(file("c:\Print\[Symbol].log"),date,symbol,close);
...
複製1print(file("C:\print\[Symbol]\"),date,symbol,close);
這樣子可以把不同腳本輸出的資料都集中到個別商品的目錄當中。



或者是，我們想把所有數據都輸出到同一個檔案print.log中，可以這樣寫：
複製1print(file("C:\print\print.log"),date,symbol,close);
...
| 1749713423-20 | https://www.xq.com.tw/lesson/xspractice/ | 探討變數序列的觀念：幾天前黃金交叉商品為例 | https://www.xq.com.tw/lesson/xspractice/%E5%A6%82%E4%BD%95%E6%89%BE%E5%87%BA%E5%B9%BE%E5%A4%A9%E5%89%8D%E9%BB%83%E9%87%91%E4%BA%A4%E5%8F%89%E7%9A%84%E5%95%86%E5%93%81%EF%BC%9A%E6%8E%A2%E8%A8%8E%E8%AE%8A%E6%95%B8%E5%BA%8F%E5%88%97%E7%9A%84/ | 探討變數序列的觀念：幾天前黃金交叉商品為例2016-03-09 22:02:04複製連結有朋友問到，系統內有提供很多找出均線黃金交叉，MACD黃金交叉的腳本，可是這些腳本好像都是要找現在黃金交叉的商品。那如果我想要找的是幾天前黃金交叉的商品，那XS辦得到嗎？今天就在這一篇文章內跟大家介紹如何做到這樣子的功能。

首先我們來看一下均線黃金交叉的腳本：
複製1input: Shortlength(5, "短期均線期數");
2input: Longlength(20, "長期均線期數");
3
4Value1 = Average(Close,Shortlength);
5Value2 = Average(Close,Longlength);
6
7If Value1 cross over Value2 then ret = 1;
...
5Value2 = Average(Close,Longlength);
6
7If Value1[1] cross over Value2[1] then ret = 1;
這樣子是不是很簡單呢？
...
2if Value1 > 0 And Value1[1] > 0 And Value1[2] > 0 then ret=1;
大家可以看得出這個腳本要找的是什麼商品了嗎？就祝大家作多的股票都跟這個腳本一樣，連續上漲三天喔！！ |
| 1749713425-21 | https://www.xq.com.tw/lesson/xspractice/ | SetBackBar指定頻率設定資料筆數 | https://www.xq.com.tw/lesson/xspractice/%E8%A8%AD%E7%BD%AE%E6%8C%87%E5%AE%9A%E9%A0%BB%E7%8E%87%E5%BC%95%E7%94%A8%E7%AD%86%E6%95%B8%E7%9A%84%E6%87%89%E7%94%A8/ | SetBackBar指定頻率設定資料筆數2021-08-19 17:01:02複製連結舊版在撰寫GetField指定跨大頻率的運算條件，例如以下範例交易腳本語法
複製1//交易策略執行頻率請設定5分鐘
2//
3//進場條件
4//收盤價在20日均線之上
5//且 5 分鐘K棒突破過去三根K棒最高價
6//
7//出場條件
8//收盤價在20日均線之下
9//且 5 分鐘K棒跌破過去三根K棒最高價
10input:Davglength(20,"日均線期數"), Hlength(3, "近N根5分K棒最高價");
11
12settotalBar(Davglength*54);
13
14condition1 = close > average(GetField("收盤價", "D"),Davglength);
15condition2 = close cross Above highest(high[1], Hlength);
16condition11= close < average(GetField("收盤價", "D"),Davglength);
17condition21= close cross under highest(high[1], Hlength);
18
19if condition1 and condition2 and filled = 0 then
20setposition(1,market);
21
22if condition11 and condition21 and filled <> 0 then 
23setposition(0,market);
...
9//且 5 分鐘K棒跌破過去三根K棒最高價
10input:Davglength(20,"日均線期數"), Hlength(3, "近N根5分K棒最高價");
11settotalBar(Hlength);
12setbarBack(Davglength,"D"); // 在 v7.06 版本添加 setbackbar 指定頻率調整資料引用筆數
13
14condition1 = close > average(GetField("收盤價", "D"),Davglength);
15condition2 = close cross Above highest(high[1], Hlength);
16condition11= close < average(GetField("收盤價", "D"),Davglength);
...
| 1749713428-22 | https://www.xq.com.tw/lesson/xspractice/ | SetTotalBar資料讀取範圍與腳本執行的關係 | https://www.xq.com.tw/lesson/xspractice/%E8%B3%87%E6%96%99%E8%AE%80%E5%8F%96%E7%AF%84%E5%9C%8D%E8%88%87%E8%85%B3%E6%9C%AC%E5%9F%B7%E8%A1%8C%E7%9A%84%E9%97%9C%E4%BF%82/ | SetTotalBar資料讀取範圍與腳本執行的關係2016-03-09 12:42:22複製連結當你在設定XS警示時，有沒有注意到有一個設定資料讀取範圍的參數，可以從這裡選擇筆數或是起始日期。這個參數的用意到底是什麼呢？在這一篇文章內就讓我們來為大家說明。
...
複製1Value1 = (Close + Close[1] + Close[2]) / 3;
...
複製1Value1 = Close - Close[5]; {計算５日漲幅}
...
| 1749713430-23 | https://www.xq.com.tw/lesson/xspractice/ |  | https://www.xq.com.tw/lesson/xspractice | XS語法應用2016-03-09 10:59:43複製連結 
