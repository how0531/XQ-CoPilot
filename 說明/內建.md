...
if Position = 1 and exit_condition then SetPosition(0);
// ```
我們如果這樣寫的話Position是0，而且entry_condition是true的時候，腳本會呼叫SetPosition(1)。
那麼腳本呼叫完SetPosition(1)之後，在下一行執行時Position會馬上變成1嗎？
// 這個問題的答案是：**不會**。
...
- **開始位置**: 執行排序的開始位置（從1開始）
- **結束位置**: 執行排序的結束位置（從1開始）
- **排序的比較欄位**: 決定排序的基準欄位（從1開始）
- **排序的順序**: `true` 由小排到大，`false` 由大排到小

**說明:**

執行後，陣列內指定範圍內的元素將依照指定欄位重新排列，同列的資料會以比較欄位為基準一起移動。

**範例:**

```xs
Array: datum[15, 6](0); // 15列 6行的二維陣列
Var: i(0);
...
Array_Sort2d(datum, 1, 15, 6, true);  // 以第6行由小到大排序
Array_Sort2d(datum, 1, 15, 6, false); // 以第6行由大到小排序
```
...
執行後，從陣列的開始位置到結束位置的每個元素都會被改成新設定的數值，範圍外的元素保持不變。
...
Array_SetMaxIndex(NumArray, Count); // 擴充陣列大小
NumArray[Count] = High;
```

**使用情境:**

上例中，我們需要儲存破20期新高的所有價格。由於無法預知創新高的次數，因此使用動態陣列儲存。當出現新高時，使用 `Array_SetMaxIndex` 擴充陣列大小。
...
Value1 = Array_GetType(arrNumber);  // Value1 = 7 (數值陣列)
Value2 = Array_GetType(arrString);  // Value2 = 3 (字串陣列)
Value3 = Array_GetType(arrBoolean); // Value3 = 2 (邏輯陣列)
```
...
可以利用這個函數來動態取得陣列的大小，讓程式更容易維護。當未來需要調整陣列大小時，迴圈程式碼不需要修改。

**範例:**

```xs
Array: arrA[5](0); // 宣告 arrA 是一個有 5 個元素的陣列
...
```xs
回傳值 = Array_Copy(陣列A, 陣列A開始複製的位置, 陣列B, 陣列B開始儲存複製資料的位置, 複製的個數);
```

**參數:**

- **陣列A**: 來源陣列
- **陣列A開始複製的位置**: 從陣列A的第幾個索引開始複製
- **陣列B**: 目標陣列
- **陣列B開始儲存複製資料的位置**: 複製到陣列B的第幾個索引
- **複製的個數**: 要複製的元素數量
...
第一個範例：從 arrA[1] 複製 5 個元素到 arrB[1]，結果 arrB 與 arrA 完全相同。

第二個範例：從 arrA[1] 複製 3 個元素到 arrC[2]，複製映射如下：
...
```xs
回傳字串 = GetSymbolInfo("商品資訊欄位名稱")
```

**說明:**

GetSymbolInfo 函數可以傳入商品資訊欄位的中文或是英文名稱，回傳對應的商品屬性。

| 欄位名稱              | 說明                            | 格式 / 範例    | 支援商品範圍                       |
| :-------------------- | :------------------------------ | :------------- | :--------------------------------- |
| **交易所** (exchange) | 商品掛牌的交易所                | 字串           | 台股、權證、可轉債、特別股、美股   |
| **累計異常注意股**    | 當日是否為累計異常注意股        | 布林值         | 台股、權證、可轉債、特別股         |
| **注意股**            | 當日是否為注意股                | 布林值         | 台股、權證、可轉債、特別股         |
| **處置股**            | 當日是否為處置股(警示股)        | 布林值         | 台股、權證、可轉債、特別股         |
| **即將處置結束股**    | 當日是否是處置期間的最後一日    | 布林值         | 台股、權證、可轉債                 |
| **近期處置結束股**    | 商品是否在過去 7 日內剛結束處置 | 布林值         | 台股、權證、可轉債                 |
| **買賣現沖**          | 當日是否可以現股當沖            | 布林值         | 台股                               |
| **先買現沖**          | 當日是否可以現股先買後賣        | 布林值         | 台股                               |
| **可放空**            | 當日是否可以融券放空            | 布林值         | 台股                               |
| **平可空**            | 當日是否可以在平盤以下融券放空  | 布林值         | 台股                               |
| **期貨近月**          | 相關期貨近月商品代碼            | FITXN01.TF     | 台股、期貨                         |
| **期貨遠月**          | 相關期貨遠月商品代碼            | FITXN02.TF     | 台股、期貨                         |
| **期貨次遠月**        | 相關期貨次遠月商品代碼          | FITXN03.TF     | 台股、期貨                         |
| **到期日**            | 商品的到期日                    | YYYYMMDD       | 權證、可轉債、期貨、選擇權、美(特) |
| **可轉換日**          | 可轉債(CB)的可轉換日期          | YYYYMMDD       | 台(可轉債)                         |
| **第一個回購日**      | 美股特別股的第一個回購日期      | YYYYMMDD       | 美(特別股)                         |
| **有認購權證**        | 是否有發行認購型權證商品        | 布林值         | 台股、大盤                         |
| **有認售權證**        | 是否有發行認售型權證商品        | 布林值         | 台股、大盤                         |
| **有牛證**            | 是否有發行牛證權證商品          | 布林值         | 台股、大盤                         |
| **有熊證**            | 是否有發行熊證權證商品          | 布林值         | 台股、大盤                         |
| **有期貨**            | 是否有衍生的期貨商品            | 布林值         | 台股、大盤                         |
| **有選擇權**          | 是否有衍生的選擇權商品          | 布林值         | 台股、大盤                         |
| **有可轉債**          | 是否有發行可轉債(CB)商品        | 布林值         | 台股                               |
| **執行比例**          | 權證商品的執行比例              | 數值           | 台(權證)                           |
| **履約價**            | 選擇權或權證商品的履約價        | 數值           | 台(權證)、選擇權                   |
| **買賣權**            | 選擇權或權證商品的買賣權別      | CALL/PUT       | 台(權證)、選擇權                   |
| **標的物**            | 衍生性商品對應的標的代碼        | 字串           | 權證、可轉債、特別股、期貨、選擇權 |
| **轉換價格**          | 可轉債(CB)最新的轉換價格        | 數值           | 台(可轉債)                         |
| **票面利率**          | 票面利率                        | 數值           | 可轉債、特別股(台/美)              |
| **擔保品**            | 可轉換商品(CB)是否有擔保        | 布林值         | 台(可轉債)                         |
| **發行張數**          | 可轉債(CB)的發行張數            | 數值           | 台(可轉債)                         |
| **交易單位**          | 商品的交易單位                  | 數值           | 台股                               |
| **面額**              | 股票每一股的面額                | 數值           | 台股                               |
| **面額幣別**          | 股票的發行幣別                  | "台幣", "美元" | 台股                               |
| **交易幣別**          | 商品的交易幣別                  | "台幣", "美元" | 台股                               |
| **ETD**               | 是否是交易所交易債券            | 布林值         | 美(特別股)                         |

**範例:**

```xs
var: aa(""), aaa("");
aa = GetSymbolInfo("交易所");      // 回傳最新的實際掛牌交易所
aaa = GetSymbolInfo("exchange");   // 亦可使用英文名稱
```
