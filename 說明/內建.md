...
```xs
Array_Copy(陣列A, 陣列A開始的位置, 陣列B, 陣列B開始儲存資料的位置, 的個數)
```
...
```xs
第一個範例內，指定從arrA的第一個位置開始到arrB的第一個位置，總共5個元素，所以執行完成後arrB的內容會是[1, 2, 3, 4, 5]，剛好跟arrA的數值完全一樣。
```


```xs
第二個範例內，指定從arrA的第一個位置開始到arrC的第二個位置，總共3個元素，也就是說:
...
```xs
所以執行完成後 arrC的內容會是 [0, 1, 2, 3, 0]，注意到arrC的初始值為0，所以沒有被到的位置還是保留初始值。
```
